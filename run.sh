...
docker run --rm -it -e "REMOTE_TARGET_NAME=BBOX" -e CLUSTER_HOST=brightbox -e NODE_COUNT=2 -e CLIENT_ID=******* -e CLIENT_SECRET=******** -e VM_SIZE=nano cakk/pmxdeploy:latest
...
#docker run --rm -it -e "REMOTE_TARGET_NAME=AMAZON" -e CLUSTER_HOST=amazon -e NODE_COUNT=2 -e AWS_ACCESS_KEY_ID=AKIAJYG5IZ5QEEFA3KQQ -e "AWS_SECRET_ACCESS_KEY=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" -e REGION=us-east-1b -e "SSH_KEY_NAME=mac" -e VM_SIZE=t1.micro cakk/pmxdeploy:latest
